...
% the circle coinciding with the center of D and Ns = 80 receivers are placed along 
% the measurement domain in an equiangular manner for each illuminations.
% Total number of different illuminations are Ni = 40.

% Some materials for simulation:
% 1.The Synthetic data: <E_lossy>
...
%The lossy profile with the relative permittivity of the disks being 3.8,SNR=20db
...
epr = ones(M,M);
epr((x-0.4).^2+(y-0.6).^2<=0.3^2) = 3.8;
epr((x+0.4).^2+(y-0.6).^2<=0.3^2) = 3.8;
epr(abs(x-0)<=0.8 & abs(y+0.3)<=0.5) = 1.5-1j*0.2;
epr(abs(x-0)<=0.5 & abs(y+0.3)<=0.3) = 2.5-1j*0.5;
chai0 = epr-ones(M,M);
chai0 = chai0(:);

WasK = zeros(M,M);
WasK((x-0.4).^2+(y-0.6).^2<=0.3^2) = 1;
WasK((x+0.4).^2+(y-0.6).^2<=0.3^2) = 1;
WasK(abs(x-0)<=0.8 & abs(y+0.3)<=0.5) = 1;
WasK = WasK(:);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%% 4.¡¾Gs¡¿%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ns = 80;
phi_temp = linspace(0,2*pi,81);phi_temp(end) = [];
phi =phi_temp;
...
%%%%%%%%%%%%% 5.¡¾<E_inc>¡¿%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ni = 40;
theta = pi/20:pi/20:2*pi; % angle of incidence
lambda = 0.75;
k0 = 2*pi/lambda;
...
iter = [200,150,50]; %%% This is the parameter defined in Ref. [21] and [22], which controls the 
                      %%% number of the iterations in each round. The example here shows that,
                      %%% in the first round, the number of iterations is 
                      %%% 200; In the second round, the number of iterations
                      %%% is 150; In the third round, the number of
                      %%% iterations is 50;
                      

                      
beta = [30,1,0.5];  %%% This is the parameter defined in Ref. [20], which is the core parameter
                     %%% being used in CIE. The example here shows that,
                     %%% in the first round, the value of \beta is 30;
                     %%% (Different from Ref. [20], the value of \beta used in the first round
                     %%% can be larger than 10; This is very crucial!)
                     %%% In the second round, the value of \beta is 1;
                     %%% In the third round, the value of \beta is 0.5;

MF = [5,7,8];       %%% This is the parameter defined in Ref. [15], which controls the 
...
% R_n = real(R_n);   % if we restrict the scatterers to be lossless as a prior information, We can uncomment this line.                      
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
%%% Display the two kinds of error in one figure
figure;      
plot(Er_tot,'r-','LineWidth',3.5);
hold on
plot(Er_sct,'b-','LineWidth',3.5);
grid on
